...
const AIAssistant: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'model', text: "Hello! I'm your SS Paw Pal assistant. How can I help you with your pet today?" }
  ]);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // We pass the conversation history to give the model context
      // Note: We filter out the initial welcome message if it wasn't generated by the API to keep history clean,
      // or we can just send the user message if we treat each turn independently, but chat context is better.
      const history = messages
        .filter((_, i) => i > 0) // Skip the hardcoded greeting for the API context if desired, or keep it.
        .map(m => ({
          role: m.role,
          parts: [{ text: m.text }]
        }));
      ...
      setMessages(prev => [...prev, { role: 'model', text: "I apologize, but I'm having trouble connecting to the network right now. Please try again in a moment." }]);
    } finally {
      setIsLoading(false);
    }
  };
            ...
            return (
              <div key={idx} className={`flex w-full ${isUser ? 'justify-end' : 'justify-start'}`}>
                <div className={`flex gap-4 max-w-[85%] md:max-w-[75%] ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
                  ...
                  {/* Bubble */}
                  <div className={`p-5 rounded-[1.5rem] text-sm font-medium leading-relaxed shadow-sm relative ${isUser ? 'bg-slate-900 text-white rounded-tr-none' : 'bg-slate-50 text-slate-800 border border-slate-100 rounded-tl-none'}`}>
                    {msg.text}
                  </div>
                </div>
              </div>
