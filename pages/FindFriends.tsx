...
import { searchUsersByUsername, followUser, unfollowUser, onFollowsUpdate, startChat } from '../services/firebase';
...
const FindFriends: React.FC = () => {
  ...
  const debouncedSearch = useCallback(debounce(async (query: string) => {
    ...
    setNoResults(false);
    try {
      const users = await searchUsersByUsername(query, user.uid);
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-10 pb-20 animate-fade-in">
      <div>
        ...
        <p className="text-slate-500 font-medium">Connect with other pet parents by searching their username.</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        {noResults && (
          <div className="text-center py-16 bg-white rounded-3xl border border-slate-100">
            ...
            <p className="text-slate-500 text-sm">Check the username and try again.</p>
          </div>
        )}
      </div>
    </div>
