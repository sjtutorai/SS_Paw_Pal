...
import { db, onFollowsUpdate } from '../services/firebase';
import { 
  collection, 
  addDoc, 
  query, 
...
const Community: React.FC = () => {
    ...
    try {
      ...
      for (const part of response.candidates[0].content.parts) {
        if (part.inlineData) {
          ...
          setSelectedImage(`data:${mimeType};base64,${part.inlineData.data}`);
  ...
  const filteredPosts = useMemo(() => {
    ...
    return result;
  }, [posts, searchQuery, sortBy, feedMode, following, user]);
      ...
      <div className="flex items-center gap-4 bg-white/50 p-2 rounded-2xl border border-slate-100 overflow-x-auto scrollbar-hide">
        <div className="relative flex-1">
          ...
          <input 
            ...
            className="w-full bg-white border border-slate-100 rounded-xl py-3 pl-12 pr-4 text-[10px] font-black uppercase tracking-widest text-slate-600 focus:ring-4 focus:ring-theme/5 outline-none transition-all shadow-sm" 
          />
        </div>
